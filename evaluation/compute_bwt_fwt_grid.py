...
def _resolve_out_dir(base_dir: Path, order: int, run_id: int) -> Path:
    """Support both standard runs (base_dir/order_*/outputs*) and grid order dirs (.../order_*/outputs*)."""
    if (base_dir / f"outputs{run_id}").is_dir():
        return base_dir / f"outputs{run_id}"
    return base_dir / f"order_{order}" / f"outputs{run_id}"


def _selected_grid_results_path(task_dir: Path) -> Optional[Path]:
    """For grid-search runs, load the best lambda candidate's all_results.json."""
    summary_path = task_dir / "lambda_search_summary.json"
    ...
    if not best_label:
        return None
    results_path = task_dir / f"lambda_{best_label}" / "all_results.json"
    return results_path if results_path.is_file() else None


def _task_results_path(out_dir: Path, task_idx: int, task: str) -> Optional[Path]:
    task_dir = out_dir / f"{task_idx}-{task}"
    # Grid-search layout: use the selected candidate's results
    grid_path = _selected_grid_results_path(task_dir)
    if grid_path is not None:
        return grid_path
    # Standard layout
    results_path = task_dir / "all_results.json"
    return results_path if results_path.is_file() else None


def load_a_tt(base_dir: Path, order: int, run_id: int, tasks: List[str]) -> List[Optional[float]]:
    """a_{t,t}: from folder {t}-{task}/all_results.json, or best grid candidate's results, metric for that task."""
    out_dir = _resolve_out_dir(base_dir, order, run_id)
    a_tt = []
    for t, task in enumerate(tasks, start=1):
        results_path = _task_results_path(out_dir, t, task)
...
def load_overall_accuracy(base_dir: Path, order: int, run_id: int, tasks: List[str]) -> Optional[float]:
    """Overall accuracy = predict_exact_match from last task folder or selected best grid candidate."""
    out_dir = _resolve_out_dir(base_dir, order, run_id)
    last_task = tasks[-1]
    results_path = _task_results_path(out_dir, len(tasks), last_task)
    if results_path is None:
        return None
    with open(results_path) as f:
        data = json.load(f)
...
def load_a_T(base_dir: Path, order: int, run_id: int, tasks: List[str]) -> List[Optional[float]]:
    """a_{T,t}: from last folder's all_results.json, or selected best grid candidate's results."""
    out_dir = _resolve_out_dir(base_dir, order, run_id)
    last_task = tasks[-1]
    results_path = _task_results_path(out_dir, len(tasks), last_task)
    if results_path is None:
...
def main(config: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    if all(x is None for x in a_tt):
        print(f"Error: no results in {_resolve_out_dir(base_dir, order, run_id)}", file=sys.stderr)
        return None, None, None
...
if __name__ == "__main__":
    base_dir = ROOT_DIR / "logs_and_outputs/ella_new_splits/long_16_2_grid"
    CONFIGS = [
        {
            "base_dir": base_dir,
            "order": 4,
