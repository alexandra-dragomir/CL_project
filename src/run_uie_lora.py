...
# Wandb: use --report_to wandb to log to Weights & Biases (default in our scripts), or --report_to none to disable.
...
def main():
    ...
    # Get the UIE dataset
    # If overwrite_cache is set, delete existing cache to force regeneration
    if data_args.overwrite_cache and os.path.exists(data_cache_dir):
        logger.info(f"Overwriting cache: removing {data_cache_dir}")
    ...
    if training_args.do_train:
        ...
        print("starting training with method:", training_args.cl_method)
...
# Default argv for "python3 run_uie_lora.py" (first task from scripts_t5/order_1.sh)
_DEFAULT_ORDER1_TASK1 = [
    "run_uie_lora.py",
    "--do_train",
    "--do_predict",
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        sys.argv = [sys.argv[0]] + _DEFAULT_ORDER1_TASK1[1:]
    main()
