    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], num_items_in_batch: int = None) -> torch.Tensor:
        ...
        for prefix, params in module_params.items():
            if all(k in params for k in ["lora_A", "lora_B", "loranew_A", "loranew_B"]):
                ...
                # ELLA loss: ||ΔW_t ⊙ W_ref||_F²; optionally drop lowest drop_lowest fraction of W_ref (by |W_ref|)
                ...
                if drop_frac > 0 and drop_frac < 1.0:
                    # Drop positions where |W_ref| is smallest; keep top (1 - drop_frac) by |W_ref|
                    flat_w = W_ref.flatten().abs()
                    n = flat_w.numel()
                    k_keep = max(1, n - int(round(n * drop_frac)))
                    _, keep_idx = flat_w.topk(k_keep, largest=True)
                    flat_sq = elem_sq.flatten()
                    ella_loss += flat_sq[keep_idx].sum()
                else:
