...
def _parse_lambda_search_by_task(value: Any) -> Optional[Dict[int, Optional[List[float]]]]:
    if value is None:
        return None
    if not isinstance(value, dict):
        raise ValueError("lambda_search_by_task must be a dict like {1: [1e4, 1e5], 2: None}")

    parsed: Dict[int, Optional[List[float]]] = {}
    for task_num, task_values in value.items():
        task_key = int(task_num)
        if task_values is None:
            parsed[task_key] = None
        else:
            parsed[task_key] = [float(x) for x in task_values]
    return parsed


def _lambda_search_candidates(cfg: Dict[str, Any], task_num: int, default_value: float) -> List[float]:
    search_by_task = cfg.get("lambda_search_by_task")
    if search_by_task is not None and task_num in search_by_task:
        task_values = search_by_task[task_num]
        if task_values is None:
            return [default_value]
        return task_values
    return _candidate_lambdas(default_value)
...
def _resolve_config(cfg: Dict[str, Any], defaults: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out["lambda_search_by_task"] = _parse_lambda_search_by_task(out.get("lambda_search_by_task"))
...
def run_grid_search_config(cfg: Dict[str, Any], dry_run: bool, gpu_id: int) -> bool:
    ...
    log_print(f"Lambda search by task: {cfg.get('lambda_search_by_task')}")
    log_print(f"{'=' * 70}\n")
    ...
    if not dry_run:
        with open(dirs["log_file"], "w", encoding="utf-8") as f:
            ...
            f.write(f"lambda_search_by_task={cfg.get('lambda_search_by_task')}\n\n")
    ...
    for task_index, task in enumerate(cfg["tasks"]):
        ...
        candidates = _lambda_search_candidates(cfg, task_num=task_num, default_value=default_lamda_1)
...
def main(configs: List[Dict[str, Any]], dry_run: bool, gpu_id: int) -> int:
    default_config = {
        ...
        "lamda_2": None,
        "lamda_1_list": None,
        "lambda_search_by_task": None,  # e.g. {1: [0, 1e4, 1e5], 2: None}; None means use only default lambda
        # "ella_drop": False,
        # "drop_lowest": 0.1,
        "train_batch_size": 16,
...
if __name__ == "__main__":
    configs = [
        {
            "order": 4,
            "run_name": "long_16_2_grid_first_base_w",
            "run_number": 1,
            "seed": 42,
            "ella_variant": "ella_first_base_w",
            "train_batch_size": 16,
            "gradient_accumulation_steps": 2,
            "lambda_search_by_task": {1: [0.0, 5e1, 5e2, 5e3, 5e4, 5e5, 5e6, 5e7],
                                    #   2: None,
                                    #   3: None,
                                    #   4: None,
                                    #   5: None,
        ...
        #     "seed": 42,
        #     "ella_variant": "ella",
        #     # "lambda_search_by_task": {1: [0.0, 1e4, 5e4, 1e5], 2: None},
        # },
    ]
